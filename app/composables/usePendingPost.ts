/**
 * usePendingPost — 发布帖子后乐观插入首页瀑布流
 *
 * 用户在 /create 页发布成功后，把刚发布的 Post 对象塞进此模块级响应式队列，
 * 同时 router.replace("/")。首页 index.vue 在挂载/缓存恢复阶段消费这个队列，
 * 把帖子 unshift 到 list 头部。
 *
 * 设计要点：
 * - 模块级 shallowRef：跨组件、跨路由共享，作为发布帖子 → 首页的单向消息总线。
 ...
 * - FIFO drain：连续多次发布帖子按发布顺序排列在列表顶部。
 */
