...
> 背景：等级系统（等级页 / 顶栏进度条 / 各处分级徽章 / 创作权益 / 签到 / 每日经验 / 米游社 zzzLevel）已于早前整体移除。现计划**重新设计**一套更简洁的等级体系，并在「论坛设置」（`/admin/forum`）提供**功能开关**：开启时显示等级，关闭时维持当前无等级状态。
>
> 核心原则：**开关归开关，数据归数据** —— 等级字段（`level`/`exp`）始终存在并随行为累计，但「是否展示 / 是否产生经验」由开关决定，保证两种状态都稳定可切换、不残留。

### 1. 功能开关

| 项 | 说明 |
|----|------|
| 字段名 | `showLevel`（`boolean`） |
| 默认值 | `false`（维持现状：无等级展示） |
| 配置位置 | `/admin/forum`「导航栏功能开关」卡片内新增「等级体系」开关 |
| 存储 | 沿用现有 `KEYS.settings` Blob 文档，前后端同步（`updateSettings` / `settings` / `publicSettings`） |
| 生效方式 | 前端 `useSiteSettings` 新增 `showLevel`，无需重新登录，刷新即生效；后端各写接口读取同一份设置 |

**开关语义**：
- **开（true）**：展示等级体系全链路（见下），行为产生经验并累计升级；
- **关（false）**：回到当前状态 —— 无 Lv 徽章、无顶栏进度条、无 `/level` 页入口、无经验累计（写接口不再加经验），但已产生的 `level`/`exp` 字段保留不显示。

### 2. 数据与计算（后端为唯一权威）

| 项 | 说明 |
|----|------|
| 用户文档 | 新增 `level`（`number`，默认 1）、`exp`（`number`，默认 0）；注册/建号时初始化 |
| 阈值表 | 沿用简化 7 级：`[0, 500, 2000, 6000, 15000, 35000, 80000]`（Level 1→7 累计绳网信用门槛） |
| 等级计算 | 由**后端**依据累计 `exp` 计算 `level` 并落库（写接口统一走 `awardExp(userId, amount)` 助手），前端不自行推导 |
| 序列化 | `toAuthor` / `toPost` / `toComment` / 个人主页 / 后台用户列表输出 `level`/`exp`（仅在开关开启时对前端有意义，关闭时前端不读取） |
| 经验来源 | 简化为一套：发帖 +4、评论 +3、点赞（收到）+1、签到 +6~10（签到本身作为独立功能可另配） |
| 日上限 | 「主动行为」每日上限 50 exp，防止刷级（可选，P2） |

### 3. 展示点（开关开启时）

| 位置 | 展示 |
|------|------|
| 顶栏左侧 | 头像 + 用户名 + 迷你等级进度条 + `LEVEL n`（沿用原 `.ik-level-display` 视觉，删除后重做精简版） |
| 帖子 / 评论 / 弹窗 | 作者名旁 `Lv.n` 小徽章 |
| 个人主页横幅 | 头像角标等级 + 经验进度 |
| 等级页 `/level` | 等级卡片（当前等级/称号/进度）+ 各等级门槛表 + 经验规则说明；桌面顶栏与移动端底部导航各加入口 |
| 用户悬停卡 / @提及 | 头像角标等级（可选） |
| 后台用户管理 | 用户列表「等级 / 经验」列（可选） |

### 4. 等级称号（可选，纯展示）

`1 新手绳匠 / 2 见习绳匠 / 3 正式绳匠 / 4 资深绳匠 / 5 精英绳匠 / 6 传奇绳匠 / 7 传说绳匠`（原 `LEVEL_TITLES`）。

### 5. 与已删除系统的取舍（重新设计的差异）

| 旧系统 | 本次取舍 |
|--------|----------|
| 创作权益矩阵（发帖/评论图片数、正文字数随等级变化） | **不恢复**。配额改为固定值（已实现：发帖 9 图 / 评论 9 图 / 正文 16000 字），避免等级影响创作功能 |
| 每日经验状态接口 `/api/me/exp/daily` | 可恢复为只读展示「今日已获 / 上限」，作为等级页信息，不参与配额 |
| 签到（签到日排名 / 丁尼奖励） | 作为独立功能可与等级解耦：签到仍可发丁尼，是否同时产生经验由开关决定 |
| 米游社 `zzzLevel`（绝区零角色等级） | **不恢复**。与本站等级无关，避免混淆 |
| 升级动画 / 进度条动效 | 保留轻量（进度条宽度过渡即可），不做旧版复杂动效 |

### 6. 实施步骤

| 阶段 | 内容 |
|------|------|
| **P1 开关打通** | 后端设置增删 `showLevel`（三处接口）+ 前端 `useSiteSettings` + `/admin/forum` 开关 UI；前后端构建验证 |
| **P2 数据与展示** | 用户文档 `level/exp` 初始化 + `awardExp` 助手 + 序列化输出；重做顶栏进度条、帖子/评论/主页 Lv 徽章 |
| **P3 等级页与规则** | 重建 `/level` 页（当前等级 + 门槛表 + 规则）+ 桌面/移动入口；后端按阈值计算等级 |
| **P4 收尾** | 经验来源接入写接口（发帖/评论/点赞/签到）、后台用户等级列、每日经验只读接口；全面回归「开关关闭=现状」 |

### 7. 风险与注意

- **存量用户**：升级开启后，存量用户 `exp=0` 全部为 Lv.1，属预期；可在开启时提供「按历史发帖/评论数补发经验」的一次性迁移（可选，P4）。
- **前端自动导入**：`useBenefits` / `useBenefitsModal` 等旧 composable 已删除，重建时勿重新引入，直接用 `useSiteSettings().settings.showLevel` + 简单 `level` 工具函数（`~/utils/level.ts`）。
- **双状态回归**：每次改动都需在「开关开 / 关」两种状态各验证一次，确保关闭时与现状逐像素一致。
