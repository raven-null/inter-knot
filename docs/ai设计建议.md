...
> 现状：/create 编辑器已具备标题/正文/分类/封面上传/草稿箱/匿名发布，桌面为「左栏导航 + 右栏编辑 + 底部操作条」，移动端为扁平编辑器 + 底部弹层。本节给出改版设计。

### 1. 改版目标与原则

| 原则 | 说明 |
|------|------|
| 聚焦写作 | 进入页面即聚焦标题，编辑区居中、干扰最小化 |
| 状态可见 | 自动保存状态、字数、审核提示随时可见 |
| 所见即所得 | 正文支持 Markdown 工具栏 + 实时预览 |
| 全端自适应 | 桌面多栏、平板折叠、手机单栏，断点平滑过渡 |
| 一次成稿 | 封面/分类/发布选项集成在侧栏或工具栏，不打断写作 |

### 2. 布局重做（响应式）

`
≥1200px  三栏：左「导航/草稿」+ 中「编辑器」+ 右「设置工具栏」
768-1199 两栏：左栏折叠为顶栏 Tab，编辑器 + 右侧工具列
<768px   单栏：编辑器铺满，分类/封面/设置收进底部弹层
`

- **中栏（编辑器）**：标题输入 → 正文编辑器（带 Markdown 工具栏）→ 封面/媒体区，自上而下单一主任务流。
- **右栏（设置工具栏，桌面）**：分类选择、匿名发布开关、封面顺序说明、发布/保存草稿按钮。
- **底栏**：自动保存状态 + 字数 + 「保存草稿」「发布」（主按钮，主题绿）。

### 3. 编辑器增强

- **Markdown 工具栏**：加粗 / 斜体 / 链接 / 引用 / 代码块 / 列表 / 图片，插入到光标位置。
- **实时预览**：分栏「编辑 | 预览」，复用 useRenderedBody（MarkdownIt + DOMPurify）实时渲染，预览区与前台详情页渲染一致。
- **字数与限制**：标题 200 字、正文按等级上限；超限红字提示 + 禁用发布。
- **自动保存**：标题/正文/封面变更后防抖保存；状态指示「保存中… / 已保存 HH:mm / 草稿 N 分钟前」；恢复上次未发布草稿。

### 4. 封面与媒体

- 封面区：拖拽排序、点击预览大图、第一张自动为封面（标注「封面」角标）。
- 上传即显示 WebP 转换提示（服务端 sharp 自动转 WebP、限 2048px、剥 EXIF）。
- 上传失败可重试；上传中显示进度与可取消。

### 5. 分类与发布选项

- 分类较多时改下拉/搜索选择（保留现有 chip 直观风格，超过 N 个折叠为下拉）。
- 匿名发布开关保留；站点开启审核时发布按钮旁提示「提交后将进入待审核」。
- 编辑已发布帖子：明确「重新发布将更新线上版本」。

### 6. 草稿箱

- 桌面左栏：草稿列表（标题 + 更新时间 + 状态），点击切换继续编辑，可删除。
- 移动端：底部「草稿箱」弹层。
- 发布成功后自动清出草稿箱。

### 7. 交互反馈

- 发布成功：跳转详情页并 toast「发布成功」；审核模式跳转首页并提示「已提交审核，请等待管理员审核」。
- 放弃修改/删除草稿：二次确认。
- 拖拽上传：全屏遮罩「释放以上传图片」提示。
- 入站考试未通过：沿用 ik-create-exam-gate 引导。

### 8. 视觉规范

沿用前台 ZZZ 语言：编辑器面板三圆角（24px 0 24px 24px）、双层描边、点阵纹理背景、主按钮主题绿 #BFFF09、正文区等宽/默认字体切换；移动端 safe-area 适配。

### 9. 技术实现要点

- **组件拆分**：EditorTitle / EditorBody（含工具栏与预览）/ EditorCover / EditorToolbar（分类/匿名/设置）/ DraftSidebar，页面只做组合与状态管理。
- **复用**：useRenderedBody（预览）、上传流程（signUpload/completeUpload）、usePendingPost（发布后乐观进首页）。
- **状态**：统一 draftState（dirty/保存中/已保存）驱动底栏与指示器。
- **断点**：768 / 1024 / 1200 三档，CSS grid 切换；移动端底部弹层复用现有 ik-mobile-sheet 动画。

### 10. 分期实施建议

| 阶段 | 内容 | 目标 |
|------|------|------|
| P1 布局 | 三栏/两栏/单栏响应式重排 + 底栏状态条 | 结构清晰、全端适配 |
| P2 编辑器 | Markdown 工具栏 + 实时预览 + 字数/保存状态 | 写作体验提升 |
| P3 媒体与选项 | 封面排序/预览、分类下拉、审核提示 | 一次成稿 |
| P4 打磨 | 草稿箱交互、视觉统一、移动端细节 | 完整可用 |
